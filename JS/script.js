document.addEventListener("DOMContentLoaded", () => {
  ...
  // --- CARROSSEL DE PRODUTOS/SERVIÇOS ---
  ...
  $$(".add-btn").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      toast(`${btn.dataset.product || "Produto"} adicionado ao carrinho`);
    });
  });
  ...
  $("#cartBtn")?.addEventListener("click", () => {
    toast(
      cart
        ? `Você tem ${cart} ${cart === 1 ? "item" : "itens"} no carrinho`
  ...
  const applyFilters = () => {
    ...
    const active = activePill ? activePill.dataset.filter : "todos";
    ...
    if (empty) {
      if (active === "servicos") {
        ...
        empty.textContent =
          "Serviços ficam na seção Cuidados. Role para agendar banho, tosa, vet e hotel.";
        scrollToId("cuidados");
      } else {
        empty.style.display = visible ? "none" : "block";
        empty.textContent =
          "Nenhum produto encontrado para essa busca. Tente outra palavra ou filtro.";
      }
    }
  };
  ...
  // --- ATALHOS DE CATEGORIAS E SERVIÇOS ---
  ...
  $$(".article-card").forEach((card) => {
    card.addEventListener("click", () => {
      const articleIdx = Number(card.dataset.article);
      ...
      if ($("#modalMeta")) $("#modalMeta").textContent = a.meta;
